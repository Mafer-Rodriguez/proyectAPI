...
const putArea = async (req, res) => {
    try {
        ...
        const requiredFields = ['name', 'description']; // Campos requeridos
...
const patchArea = async (req, res) => {
    try {
        ...
        // Actualización parcial del área
        await area.update(req.body); // Esto actualizará solo los campos proporcionados en req.body

        // Recuperar el área actualizado después de la actualización parcial
        const updatedArea = await Area.findByPk(id);

        return res.status(200).json({
            code: 200,
            msg: "Area updated successfully",
            body: updatedArea, // Devolver el área actualizado
        });
    } catch (error) {
        const response = getResponse(500);
        res.status(response.code).json({ message: response.msg });
